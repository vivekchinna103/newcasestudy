...
def add_file(request):
    if request.method=="POST":
        ...
        name=bodyContent.CaseStudyName
        account=bodyContent.get('Account')
        vertical=bodyContent.get('Vertical')
        solution=bodyContent.get('SolutionName')
        sof=bodyContent.get('ServiceOfferingMapping')
        status=bodyContent.get('Status')
        dep=bodyContent.get('Dependency')
        remarks=bodyContent.get('Remarks')
        meta=bodyContent.get('MetaData')
        FileName=bodyContent.get('FileName')
        rating=bodyContent.get('Rating')
        file=request.FILES[FileName]
        if (file!=None):
            file_name= file.name
            file_path=file.temporary_file_path()
        add_data(name,account,vertical,solution,sof,status,dep,remarks,meta,FileName,rating)
